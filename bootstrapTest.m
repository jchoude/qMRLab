function bootstrapTest(platform)
  addpath(genpath(pwd));

  if strcmp(platform,'TRAVIS')
    dirOctave = '/home/travis/octave';
    ...
    if moxunit_util_platform_is_octave
      if ~cacheState
        ...
        for ii=1:length(installist)
          pkg('prefix',dirOctave);
          pkg('local_list',[dirOctave '/.octave_packages']);
          try
  ...
  elseif strcmp(platform,'AZURE')
    disp('Lets see how good docker and azure is');
    pkg list
    chk = test('lsqcurvefit');
    if ~chk
      error('Lsqcurvefit could not be loaded properly');
